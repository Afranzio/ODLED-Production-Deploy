    ...
    listItem.map(each => {
      if(each.fileType !== "text" && each.texts === ""){
        if(each.file !== null && each.evn ){
          const uploadTask = storage.ref(`${accName}/${new Date().getTime() + each.evn.name}`).put(each.evn);
          uploadTask.on(
              "state_changed",
              (snapshot) => {
                //prograss Bar Math
                      ...
                      <div className="changeDisplay">
                        <div className="preview" >
                          ...
                          <div style={{ width: ratios.w/value, height: ratios.h/value, border: "1px solid black"}} >
                            <Viewer setcontext={contexted} tryList={listItem} ratioStyle={ratios} usrpos={userPosition} usrsize={userSize} decide={value} removeDiv={deleteDiv} change={items} dragged={setPosition} resized={setSize} bgc={BGC} bgi={BGI} usr={accName} />
                          </div>
                        </div>
                      </div>
                  </div>
