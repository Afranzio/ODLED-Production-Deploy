...
export default function CustomDesign({exists, setexist, template, ratios, user, allData, settmp, display, cleaner}){

    const [data, setData] = React.useState(Temp2)
    const [upload, setUpload] = React.useState(0)
    const [loading, setLoading] = React.useState( upload !== 0 ? true : false)

    const [temp, setTemp] = React.useState(false)

    const [dragpoint, setDragpoint] = React.useState(true)

    const [templateNum, setTemplateNum] =React.useState(template)
    ...
    useEffect(() => {
        ...
        getData()
      },[exists, setexist, templateNum, allData, settmp]);
    ...
    const getData = () => {
        if(allData){
            if(Object.keys(allData).length !== 0){
                Object.keys(allData).map(each => {
                    each !== "Obj" ? 
                    setData(
                        Object.keys(allData).map(each => ({
                            timestamp : allData[each].timestamp,
                            id: `${each}`,
                            size: allData[each].size,
                            position: allData[each].position,
                            imageURL: allData[each].imageURL,
                            fileType: allData[each].fileType,
                            texts: allData[each].texts,
                            ratio: allData[each].ratio,
                            interval: allData[each].interval ? allData[each].interval : 1000,
                            transitionTime: allData[each].transitionTime ? allData[each].transitionTime : 350,
                            axis: allData[each].axis ? allData[each].axis : "horizontal",
                            file: {}
                        }))
                    )
                    : 
    ...
    const getDownload = () => {
        if(Object.keys(data).length !== 0){
            Object.keys(data).map(each =>{
                    if(data[each].id !== '' && data[each].id !== undefined){    
                        if(!data[each].file.name){
                            console.log(data[each].imageURL)
                            if(data[each].imageURL === ["/static/media/Dummy.fdd8279e.jpg"]){
                                db.collection(user).doc(display).set({
                                    ...
                                    id: data[each].id,
                                    interval: data[each].interval ? data[each].interval : 1,
                                    transitionTime: data[each].transitionTime ? data[each].transitionTime : 500,
                                    axis: data[each].axis ? data[each].axis : "horizontal",
                                }
                                }, { merge: true })                                   
                            }
                            else{
                        ...
                        .then(function(docRef) {
                            setID("Obj"+ String(Object.keys(data).length+1), each)
                        });
                        console.log("text")
                    }
                    else if(data[each].id == undefined || data[each].id == ""){
                        if(data[each].file.name){
                            let prefix = new Date().getTime()
                            const uploadTask = storage.ref(`${user}/${ prefix + data[each].file.name}`).put(data[each].file);
                            uploadTask.on(
                        ...
                        {Object.keys(data).map(each => {
                            return(
                                <Rnd
                                    ...
                                    bounds='parent'
                                    size={{
                                        width: Number((data[each].size.width / value) * templatesize),
                                        height: Number((data[each].size.height / value) * templatesize)
                                    }}
                                    position={{
                                ...
                                >
                                    <FSTemplate drags={setDragpoint} mngFile={addFile} mngTxt={addText} values={value} tmpSize={templatesize} setpixs={userPixel} setpos={userPosition} setMaster={masterHandler} obj={each} datas={data[each]} />
                                </Rnd>
                            )
                        })}
                    </div>
